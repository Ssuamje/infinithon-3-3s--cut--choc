...
function createWarningWindow() {
    ...
    if (process.env.VITE_DEV_SERVER_URL) {
        ...
        warningWin.webContents.once("did-finish-load", () => {
            warningWin?.webContents.executeJavaScript("window.location.hash = '#/warning'");
        });
    }
    // 경고 창이 닫힐 때 정리
...
// 깜빡임 경고 창 표시/숨김
function toggleWarningWindow(show) {
...
// 깜빡임 경고 상태 업데이트
function updateWarningState(newState) {
    ...
    console.log(`Should show warning: ${shouldShow} (isVisible: ${warningState.isVisible}, progress: ${warningState.progress})`);
